...
/*   Updated: 2021/04/15 11:43:32 by abdel-ke         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*push_read_apnd_strct(t_parse *parse, char *line, char **file, int *i)
{
	if (line[(*i) + 1] == '>')
	{
		(*i)++;
...
char	*push_file_struct(t_parse *parse, char *line, int i)
{
	...
	while (line[i])
	{
		if (line[i] == '<')
		{
			...
			line = push_read_apnd_strct(parse, line, &file, &i);
		i++;
	}
	return (line);
}
